...
bool rdmaFetchAndAddBoundary(ibv_qp *qp, uint64_t source, uint64_t dest,
  ...
  Debug::notifyError("rdmaFetchAndAddBoundary, it should not be reached!!");
  exit(-1);
// 没人调用不管
  // struct ibv_sge sg;
  // struct ibv_send_wr wr;
  ...
  // wr.exp_op.masked_atomics.log_arg_sz = 3;
  // wr.exp_op.masked_atomics.remote_addr = dest;
  // wr.exp_op.masked_atomics.rkey = remoteRKey;
...
bool rdmaCompareAndSwapMask(ibv_qp *qp, uint64_t source, uint64_t dest,
  ...
  Debug::notifyError("rdmaCompareAndSwapMask, it should not be reached!!");
  exit(-1);
  // struct ibv_sge sg;
  // struct ibv_exp_send_wr wr;
  // struct ibv_exp_send_wr *wrBad;
